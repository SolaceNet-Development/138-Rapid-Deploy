...
class GasMonitor {
  ...
  async checkGasAlerts(): Promise<void> {
    ...
    if (
      this.metrics.predictions.confidence < this.config.thresholds.predictionConfidenceThreshold
    ) {
  ...
  async reportMetrics(): Promise<void> {
    ...
    console.log('Gas Metrics:', {
      ...
      optimizationMetrics: {
        ...
        recommendedBaseFee: ethers.utils.formatUnits(
          this.metrics.optimizationMetrics.recommendedBaseFee,
          'gwei'
        ),
        recommendedPriorityFee: ethers.utils.formatUnits(
          this.metrics.optimizationMetrics.recommendedPriorityFee,
          'gwei'
        )
      }
    });
  ...
  private async updateOptimizationMetrics(): Promise<void> {
    ...
    this.metrics.optimizationMetrics = {
      ...
      estimatedSavings:
        ((parseFloat(ethers.utils.formatUnits(this.metrics.currentPrice, 'gwei')) -
          (baseFee + priorityFee)) /
          parseFloat(ethers.utils.formatUnits(this.metrics.currentPrice, 'gwei'))) *
        100,
      optimalSubmissionTime:
        this.metrics.statistics.trend === 'decreasing' ? Date.now() + 300000 : Date.now() // Wait 5 minutes if decreasing
    };
  }
}
