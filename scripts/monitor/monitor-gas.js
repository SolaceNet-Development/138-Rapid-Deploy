...
class GasMonitor {
        ...
        while (true) {
            try {
                ...
                await new Promise(resolve => setTimeout(resolve, 
                    parseInt(process.env.GAS_MONITORING_INTERVAL) * 1000 || 15000
                ));
            } catch (error) {
    ...
    calculateMedianPrices(prices) {
        ...
        for (const category of Object.keys(result)) {
            ...
            result[category] = values.length % 2 === 0
    ...
    updateMovingAverages(price) {
        ...
        if (this.movingAverages.short.length > 20) { // 5 minutes at 15-second intervals
            this.movingAverages.short.shift();
        }

        // Update 1-hour MA
        this.movingAverages.medium.push(price);
        if (this.movingAverages.medium.length > 240) { // 1 hour at 15-second intervals
            this.movingAverages.medium.shift();
        }

        // Update 4-hour MA
        this.movingAverages.long.push(price);
        if (this.movingAverages.long.length > 960) { // 4 hours at 15-second intervals
    ...
    updateStatistics() {
        ...
        // Weekly statistics
        const weeklyPrices = this.metrics.historical
            .map(data => parseFloat(data.standard));
    ...
    async checkGasAlerts() {
        ...
        const fiveMinChange = (currentPrice - this.calculateAverage(this.movingAverages.short)) / 
            this.calculateAverage(this.movingAverages.short) * 100;
