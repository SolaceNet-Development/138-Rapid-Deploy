...
class ProtocolMonitor {
    ...
    async updateAaveMetrics() {
        ...
        this.metrics.aave.utilizationRate = reserves.reduce(
            (total, { data }) => total + data.utilizationRate,
            0
        ) / reserves.length;
    ...
    async updateDodoMetrics() {
        ...
        this.metrics.dodoex.liquidityUtilization = poolData.reduce(
            (total, { data }) => total + data.utilization,
            0
        ) / poolData.length;
    ...
    async updateMakerMetrics() {
        ...
        this.metrics.maker.collateralRatio = cdpData.reduce(
            (total, { info }) => total + info.collateralRatio,
            0
        ) / cdpData.length;
    ...
    async checkLidoAlerts() {
        ...
        const lowPerformers = Object.entries(this.metrics.lido.validatorPerformance)
    ...
    async checkMakerAlerts() {
        // Check risky vaults
        const riskyVaults = Object.entries(this.metrics.maker.vaultMetrics)
            .filter(([_, metrics]) => metrics.collateralRatio < 1.5);
    ...
    async checkMETHAlerts() {
        // Check validator performance
        const lowPerformers = Object.entries(this.metrics.meth.validatorPerformance)
            .filter(([_, performance]) => performance < 0.9);
