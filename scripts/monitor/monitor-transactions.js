...
class TransactionMonitor {
    ...
    async handleFailedTransaction(tx, receipt) {
        ...
        // Check failure rate
        const recentFromAddress = this.metrics.transactions.recent
    ...
    async updateTransactionStats() {
        ...
        const hourlyTxs = this.metrics.transactions.recent
            .filter(tx => tx.timestamp * 1000 > oneHourAgo);
        const dailyTxs = this.metrics.transactions.recent
    ...
    calculateStats(transactions) {
        ...
        if (transactions.length > 0) {
            ...
            const totalValue = transactions
                .reduce((sum, tx) => sum.add(tx.value), ethers.BigNumber.from(0));
    ...
    async monitorTransactions() {
        while (true) {
            try {
                ...
                const now = Date.now();
                this.metrics.transactions.pending = this.metrics.transactions.pending
                    .filter(tx => now - tx.timestamp < this.thresholds.pendingTxAge * 1000);
                ...
                await new Promise(resolve => setTimeout(resolve, 
                    parseInt(process.env.TRANSACTION_MONITORING_INTERVAL) * 1000 || 30000
                ));
            } catch (error) {
