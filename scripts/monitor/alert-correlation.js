...
class AlertCorrelation extends EventEmitter {
  ...
  initializeDefaultRules() {
    ...
    this.addCorrelationRule('repeated-failures', {
      condition: (alerts) => {
        const failedTransfers = alerts.filter(
          (a) => a.type === 'Transfer Failed' && Date.now() - a.timestamp < 3600000 // Last hour
        );
    ...
    this.addCorrelationRule('bridge-congestion', {
      condition: (alerts) => {
        const congestionIndicators = alerts.filter(
          (a) =>
    ...
    this.addCorrelationRule(name, {
      condition: (alerts) => {
        const relevantAlerts = alerts.filter(
          (a) =>
            Date.now() - a.timestamp < timeWindow && conditions.every((condition) => condition(a))
        );

        if (relevantAlerts.length >= minOccurrences && relevantAlerts.length <= maxOccurrences) {
          return [
            {
