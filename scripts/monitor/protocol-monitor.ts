...
class ProtocolMonitor {
  ...
  private setupEventListeners(): void {
    ...
    this.contracts.lending.on('Supply', async (token, user, amount) => {
      const currentSupply =
        this.metrics.lendingMetrics.supplyVolume.get(token) || ethers.BigNumber.from(0);
    ...
    this.contracts.lending.on('Borrow', async (token, user, amount) => {
      const currentBorrow =
        this.metrics.lendingMetrics.borrowVolume.get(token) || ethers.BigNumber.from(0);
    ...
    this.contracts.trading.on('Trade', async (dex, token0, token1, amount) => {
      const currentVolume =
        this.metrics.tradingMetrics.volumeByDex.get(dex) || ethers.BigNumber.from(0);
  ...
  private async updateMetrics(): Promise<void> {
    ...
    this.metrics.performanceMetrics.transactionLatency = await this.measureTransactionLatency();
    this.metrics.performanceMetrics.gasUsage = await this.provider.getGasPrice();
  ...
  private updateHealthScore(): void {
    ...
    if (
      this.metrics.lendingMetrics.collateralizationRatio <
      this.config.thresholds.minCollateralizationRatio
    ) {
      score -=
        20 *
        (1 -
    ...
    if (
      ...
      this.config.thresholds.maxTransactionLatency
    ) {
      score -=
        20 *
        (this.metrics.performanceMetrics.transactionLatency /
