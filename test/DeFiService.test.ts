...
describe('DeFiService', () => {
  ...
  describe('Automated Strategies', () => {
    ...
    it('should execute strategy trades', async () => {
      ...
      const strategy = await env.contracts.strategyFactory.getStrategy(env.wallets[0].address, 0);
  ...
  describe('Portfolio Analytics', () => {
    it('should calculate advanced metrics correctly', async () => {
      ...
      const metrics = await defiService.getAdvancedPortfolioMetrics(env.wallets[0].address);
...
async function setupTestPortfolio(env: any, defiService: DeFiService) {
  // Add liquidity
  await defiService.addLiquidity(env.contracts.pool.address, '100', '100');

  // Open positions
  await defiService.openPerpetualPosition(env.contracts.perpetualMarket.address, true, '1.0', 5);
...
async function updateTestPrices(env: any) {
  await env.contracts.priceOracle.setPrice(utils.parseEther('1.1'));
}
